<template>
	...
	<view class="content" :style="{height:(phone.safeArea.top + contentTop)+'px'}">
		...
		<view class="mask" v-if="loading" style="background-color:#20202C;z-index:8!important;"
			@touchmove.stop.prevent="moveHandle"
			:style="{'margin-top':(contentTop-10+h5Top)+'px','height':loadingHieght+'px'}">
		...
		<view class="center-content"
			:style="{'margin-top':(phone.safeArea.top + contentTop + 95)+'px','padding-bottom':'55px'}">
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			_self.top = 10;
			_self.contentTop = _self.top + 130;
		...
		methods: {
			...
			getData() {
				if (_self.page > 1) uni.showLoading({
					title: '正在加载...',
					mask: true
				});
				uni.request({
