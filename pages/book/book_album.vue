<template>
	<view class="content">
		...
		<view class="center-content" :style="'padding-top:'+(top)+'px'">
			<view class="content-head">
				<view class="content-head-left">
					<image :src="detail.url" mode="aspectFill"></image>
				</view>
				<view class="content-head-content">
					<view class="content-head-title">
						<text>{{detail.title}}</text>
					</view>
					<view class="content-head-desc">
						<text>{{detail.description}}</text>
					</view>
					<view class="content-head-count">
						<text>{{detail.count}}部小说</text>
						<text style="padding-left:10px;">已有{{detail.buy_num}}人购买</text>
					</view>
				</view>
			</view>
			<view class="content-body">
				<view class="buy_btn" @click="openPop(detail)" v-if="detail.alreadyBuy == 0 && detail.price > 0 ">
					<text>￥{{detail.price || 0}}元购买{{detail.count || 0}}部</text>
				</view>
				...
				<view class="content-body-items">
					<block v-for="(v, k) in bookList" :key="k">
						<view class="book-list">
							<view class="book-item" @click="openDetail(v)">
								<text class="book-title">{{v.title}}</text>
								<text class="book-read">{{v.click}}次阅读</text>
								<text class="book-description" v-if="v.description.length > 0">{{v.description}}</text>
							</view>
						</view>
					</block>
			...
			<view class="notice-bg">
				...
				<view class="notice_content">
					...
					<view class="notice_content_line_3">
						<text class="buy_btn_small">立即支付</text>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			openDetail(v) {
				if (v.is_free == 1 || _self.detail.alreadyBuy == 1) {
					api.jumpUrl('/pages/book/book_album_detail?id=' + v.id, 'new')
				} else {
					_self.openPop(_self.detail)
				}
			},
			openPop(v) {
			...
			getData() {
				uni.request({
					url: api.apiData.getBookAlbum + '/userId/' + _self.userId + '/id/' + _self.id + '/page/' +
						_self.page + '/order/' + _self.tabId,
					method: 'GET',
					header: {
						'Content-type': 'application/x-www-form-urlencoded'
					},
...
<style>
	...
	.content-head-left image {
		width: 100px;
		height: 100px;
		border-radius: 10px;
		border: #20202C 1px solid;
		overflow: hidden;
	}
