<template>
	<view class="bg">
		<view class="content-head" :style="{ 'padding-top': top + 'px' }">数字钱包</view>
		...
		<view class="bank-list">
			<view class="bank-info" v-for="(b, i) in userBank" :key="i">
				...
				<view class="bank-content">
					<image class="bgx" src="@/static/imgs/delx.png" mode="aspectFill" @click="delBank(b.id)"></image>
					<view @click="setBank(b.id)">{{b.bank}}</view>
					<view style="margin-top:10px;font-size:28upx;" @click="setBank(b.id)">**** **** ****
						{{b.account.substring(b.account.length - 4,b.account.length)}}</view>
					<image class="bgimg" :src="b.icon" mode="aspectFill" @click="setBank(b.id)"></image>
				</view>
			</view>
		...
		<view class="add-bank" @click="skipUrl('/pages/member/add_digit', 2)">+ 添加数字钱包</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getUserBank() {
				uni.request({
					url: api.apiData.getUserBank + '/type/3/userId/' + _self.userId,
					method: 'GET',
					header: {
