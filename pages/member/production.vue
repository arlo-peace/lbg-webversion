<template>
	<view class="content">
		...
		<view class="production-list" v-else :style="{'margin-top':top+'px'}">
			<div class="production-info" v-for="(s, i) in productionList" :key="i">
				<image @click="goProduction(s.id)" class="production-img" :src="s.thumbnail" mode="aspectFill"></image>
				<div @click="goProduction(s.id)" class="production-title">
					{{s.name}}
				</div>
				<image @click="focusProduction(s.id)" :src="`/static/production/guanzhu002.png`"
		...
		methods: {
			...
			getProduction() {
				...
				uni.request({
					...
					success: (e) => {
						_self.productionList = e.data.Data;
						if (_self.productionList.length) {
			...
			goProduction(productionId) {
				var param = {
					url: '/pages/member/production',
					t: 'new'
				}
				api.jumpUrl('/pages/production/detail?id=' + productionId + '&param=' + JSON.stringify(param), 'new');
			},
			// 跳转登录
