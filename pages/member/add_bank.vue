<template>
	<view class="bg" :style="{ height: bgHeight + 'px'}">
		<view class="content-head" :style="{ top: top + 'px' }">添加银行卡</view>
		...
		<view class="bank-list">
			...
			<view class="bank-info">
				<input type="text" name="bankName" @input="onKeyInputNname" placeholder="请输入持卡人姓名" maxlength="15" />
			</view>
			<view class="bank-info">
				<input type="number" name="bankSn" @input="onKeyInput" placeholder="请输入卡号" />
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			addBank() {
				if (_self.bankId == 0) {
					_self.msgData('请选择银行卡', 2000);
					return false;
				}
				if (!_self.bankUsername) {
					_self.msgData('请输入持卡人姓名', 2000);
					return false;
				}
				if (_self.cardNum == 0) {
					_self.msgData('请输入银行卡账号', 2000);
					return false;
				}
				_self._showLoading('加载中...');
				uni.request({
					url: api.apiData.addBank,
					method: 'POST',
					data: {
						userId: _self.userId,
					...
					success: (e) => {
						var res = e.data;
						if (res.Code == 200) {
							_self.skipUrl('/pages/member/bank_list', 2);
						} else {
