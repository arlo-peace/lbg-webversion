<template>
	<view class="bg">
		<view class="content-head" :style="{ 'padding-top': (top+7) + 'px' }">
			...
			<view style="text-align:center;width:89%;">账户明细</view>
			<view style="margin-top:30px;font-size:50upx;">
				<!-- <text style="font-size:12px;margin-right:10upx;color:rgba(255,12,208,0.8);" v-if="tabId==1">账户金币</text>
				<text style="font-size:12px;margin-right:10upx;color:rgba(255,12,208,0.8)" v-else>账户余额</text> -->
				{{point}}
			</view>
			<view class="head-title">
				<text :class="tabId==1?'sel':''" style="border-radius:10upx 0 0 0;" @click="setTabId(1)">金币</text>
				<text :class="tabId==2?'sel':''" style="border-radius:0 10upx 0 0;" @click="setTabId(2)">余额</text>
			</view>
		</view>
		...
		<view class="bank-list" :style="{'margin-top': (top+173) + 'px'}">
			<view class="bank-info" v-for="(d, i) in logList" :key="i">
				<view class="bank-content">
					<view class="info-title">{{d.remarks}}</view>
					<view class="bank-name">{{d.addTime}}</view>
					<view class="money">
						<view style="color:#ff5e63;">{{d.point}}</view>
