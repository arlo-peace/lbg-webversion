...
function move(nowname, newdir) {
	// 兼容IOS
	nowname = nowname.replace(/_downloads\//, "")
    plus.io.requestFileSystem(plus.io.PUBLIC_DOWNLOADS, function(fobject) {
        fobject.root.getFile(nowname, {}, function(fileEntry) {
            ...
            movenum --;
			console.log(e)
            console.log('error to move:', nowname)
        });
    })
}
            ...
            if (line.startsWith('#EXT')) {
				...
				if (line.indexOf('X-STREAM-INF') != -1) {
					...
					if(keyMatch){
						...
						var p = option.url.lastIndexOf("/") + 1;
						var keyurl = option.url.substring(0, p)
						let key = {
						    url: keyurl+keyMatch[1],
						    path: path + '/' + id + '/' + keyMatch[1],
						    fullpath: fullpath + '/' + id + '/' + keyMatch[1]
						}
						console.log('key', key)
						data.tslist.push(key)
						data.newm3u8 = data.newm3u8.replace('"key.key"', "'"+key.fullpath+"'")
						// i++
					}
